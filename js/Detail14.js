document.addEventListener('DOMContentLoaded', () => {
    ...
    if (addToCartButton && quantityInput) {
        addToCartButton.addEventListener('click', function(event) {
            ...
            const quantity = parseInt(quantityInput.value);
            const unitPrice = 25000; // Giá tiền cố định cho sản phẩm
            ...
            let cart = JSON.parse(localStorage.getItem('detail14')) || [];
            ...
            localStorage.setItem('detail14', JSON.stringify(cart));
    ...
    if (localStorage.getItem('detail144')) {
        if (noProductDiv) {
            noProductDiv.style.display = 'none';
        }

        const cart = JSON.parse(localStorage.getItem('detail144'));
        ...
        cart.forEach((product, index) => {
            ...
            function updateProductQuantity(index, quantity) {
                ...
                localStorage.setItem('detail144', JSON.stringify(cart));
