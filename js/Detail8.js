document.addEventListener('DOMContentLoaded', () => {
    ...
    if (addToCartButton && quantityInput) {
        addToCartButton.addEventListener('click', function(event) {
            ...
            const quantity = parseInt(quantityInput.value);
            const unitPrice = 175000; // Giá tiền cố định cho sản phẩm
            ...
            let cart = JSON.parse(localStorage.getItem('detail8')) || [];
            ...
            localStorage.setItem('detail8', JSON.stringify(cart));
            Swal.fire({
                title: "Tuyệt!",
    ...
    if (localStorage.getItem('detail8')) {
        if (noProductDiv) {
            noProductDiv.style.display = 'none';
        }

        const cart = JSON.parse(localStorage.getItem('detail8'));
