document.addEventListener('DOMContentLoaded', () => {
    ...
    if (addToCartButton && quantityInput) {
        addToCartButton.addEventListener('click', function(event) {
            ...
            const quantity = parseInt(quantityInput.value);
            const unitPrice = 215000; // Giá tiền cố định cho sản phẩm
            ...
            let cart = JSON.parse(localStorage.getItem('detail10')) || [];
            ...
            localStorage.setItem('detail10', JSON.stringify(cart));
    ...
    if (localStorage.getItem('detail100')) {
        if (noProductDiv) {
            noProductDiv.style.display = 'none';
        }

        const cart = JSON.parse(localStorage.getItem('detail100'));
        ...
        cart.forEach((product, index) => {
            ...
            function updateProductQuantity(index, quantity) {
                ...
                localStorage.setItem('detail100', JSON.stringify(cart));
