document.addEventListener('DOMContentLoaded', () => {
    ...
    if (addToCartButton && quantityInput) {
        addToCartButton.addEventListener('click', function(event) {
            ...
            let cart = JSON.parse(localStorage.getItem('detail7')) || [];
            ...
            localStorage.setItem('detail7', JSON.stringify(cart));
            Swal.fire({
                title: "Tuyệt!",
    ...
    if (localStorage.getItem('detail7')) {
        if (noProductDiv) {
            noProductDiv.style.display = 'none';
        }

        const cart = JSON.parse(localStorage.getItem('detail7'));
