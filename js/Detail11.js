document.addEventListener('DOMContentLoaded', () => {
    ...
    if (addToCartButton && quantityInput) {
        addToCartButton.addEventListener('click', function(event) {
            ...
            const quantity = parseInt(quantityInput.value);
            const unitPrice = 205000; // Giá tiền cố định cho sản phẩm
            ...
            let cart = JSON.parse(localStorage.getItem('detail11')) || [];
            ...
            localStorage.setItem('detail11', JSON.stringify(cart));
    ...
    if (localStorage.getItem('detail111')) {
        if (noProductDiv) {
            noProductDiv.style.display = 'none';
        }

        const cart = JSON.parse(localStorage.getItem('detail111'));
        ...
        cart.forEach((product, index) => {
            ...
            function updateProductQuantity(index, quantity) {
                ...
                localStorage.setItem('detail111', JSON.stringify(cart));
