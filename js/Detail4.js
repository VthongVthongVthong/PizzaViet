document.addEventListener('DOMContentLoaded', () => {
    ...
    if (addToCartButton && quantityInput) {
        addToCartButton.addEventListener('click', function(event) {
            ...
            let cart = JSON.parse(localStorage.getItem('detail4')) || [];
            ...
            localStorage.setItem('detail4', JSON.stringify(cart));
    ...
    if (localStorage.getItem('detail4')) {
        if (noProductDiv) {
            noProductDiv.style.display = 'none';
        }

        const cart = JSON.parse(localStorage.getItem('detail4'));
