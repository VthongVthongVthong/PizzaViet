document.addEventListener('DOMContentLoaded', () => {
    ...
    if (addToCartButton && quantityInput) {
        addToCartButton.addEventListener('click', function(event) {
            ...
            const quantity = parseInt(quantityInput.value);
            const unitPrice = 20000; // Giá tiền cố định cho sản phẩm
            ...
            let cart = JSON.parse(localStorage.getItem('detail13')) || [];
            ...
            localStorage.setItem('detail13', JSON.stringify(cart));
    ...
    if (localStorage.getItem('detail133')) {
        if (noProductDiv) {
            noProductDiv.style.display = 'none';
        }

        const cart = JSON.parse(localStorage.getItem('detail133'));
        ...
        cart.forEach((product, index) => {
            ...
            function updateProductQuantity(index, quantity) {
                ...
                localStorage.setItem('detail133', JSON.stringify(cart));
