document.addEventListener('DOMContentLoaded', () => {
    ...
    if (addToCartButton && quantityInput) {
        addToCartButton.addEventListener('click', function(event) {
            ...
            let cart = JSON.parse(localStorage.getItem('detail5')) || [];
            ...
            localStorage.setItem('detail5', JSON.stringify(cart));
    ...
    if (localStorage.getItem('detail5')) {
        if (noProductDiv) {
            noProductDiv.style.display = 'none';
        }

        const cart = JSON.parse(localStorage.getItem('detail5'));
