document.addEventListener('DOMContentLoaded', () => {
    ...
    if (addToCartButton && quantityInput) {
        addToCartButton.addEventListener('click', function(event) {
            ...
            let cart = JSON.parse(localStorage.getItem('detail15')) || [];
            ...
            localStorage.setItem('detail15', JSON.stringify(cart));
            Swal.fire({
                title: "Tuyệt!",
    ...
    if (localStorage.getItem('detail15')) {
        if (noProductDiv) {
            noProductDiv.style.display = 'none';
        }

        const cart = JSON.parse(localStorage.getItem('detail15'));
