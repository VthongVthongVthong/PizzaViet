...
function showTop5() {
    ...
    rows.forEach((row, index) => {
        ...
        button.textContent = "Hiển thị tất cả"; // Thay đổi nội dung nút thành "Ẩn Top 5"
    } else {
        // Hiển thị tất cả các dòng khi nút ở trạng thái "Ẩn Top 5"
        rows.forEach(row => {
    ...
    // Recalculate total revenue based on the first 5 rows
    let totalExpense = 0;
    for (let i = 0; i < 5; i++) {
        const row = rows[i];
        const revenueCell = row.cells[4]; // Column 5 contains revenue
        const revenue = parseFloat(revenueCell.textContent.replace(/\./g, '').trim());
        totalExpense+= revenue;
    }

    // Display the total revenue for the top 5
    document.getElementById('totalExpense').textContent = totalExpense.toLocaleString();
}
...
function updateTotalExpense() {
    ...
    rows.forEach(function(row) {
        ...
        if (expenseCell) {
            var expense = parseFloat(expenseCell.innerText.replace('₫', '').replace(',', '').trim());
            if (!isNaN(expense)) {
                total += expense;
            }
        }
