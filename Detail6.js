  ...
  form.addEventListener('submit', function(event) {
    ...
    if (addToCartButton && quantityInput) {
        addToCartButton.addEventListener('click', function(event) {
            ...
            let cart = JSON.parse(localStorage.getItem('detail6')) || [];
            ...
            localStorage.setItem('detail6', JSON.stringify(cart));
            Swal.fire({
                title: "Tuyệt!",
    ...
    if (localStorage.getItem('detail6')) {
        if (noProductDiv) {
            noProductDiv.style.display = 'none';
        }

        const cart = JSON.parse(localStorage.getItem('detail6'));
